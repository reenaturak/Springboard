...
) AS 'Member Name', Facilities.name AS 'Court Name'
FROM Members
JOIN Facilities
JOIN Bookings
...
SELECT CONCAT(f.name, ', ', mem.firstname) AS facility_member,
	   CASE WHEN mem.firstname LIKE '%guest%' THEN f.guestcost * book.slots
	        WHEN mem.firstname NOT LIKE '%guest%' THEN f.membercost * book.slots
	        END AS cost
	FROM Members mem
	JOIN Bookings book 
   ...
   WHERE book.starttime LIKE '%2012-09-14%'
     AND (CASE WHEN mem.firstname LIKE '%guest%' THEN f.guestcost * book.slots
	        WHEN mem.firstname NOT LIKE '%guest%' THEN f.membercost * book.slots
	        END) > 30
   ORDER BY cost DESC 
